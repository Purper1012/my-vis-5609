<script lang="ts">
  ...
  // Line generator creates SVG path data from our year count array
  ...
  function handleBrush(event: any) {
    // Get the selected pixel range and convert to dates
    const selection = event.selection;
    if (selection) {
      // Convert pixel positions to dates using xScale
      const [start, end] = selection.map(xScale.invert);
      yearRange = [start, end];
    } else {
      // Clear the filter when brush is removed
      yearRange = undefined;
    }
  }
...
<svg {width} {height} class="line">
  <!-- The curved line showing movie count trends over time -->
  <path d={path} fill="none" stroke="steelblue" stroke-width="2" />

  <!-- Optional circles to validate the curve matches the data points -->
  <g class="points">
